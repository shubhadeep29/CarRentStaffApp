...
export default class ValidateStepFiveScreen extends React.Component {
  ...
  componentDidMount = async () => {
    ...
    if (this.state.item && Object.keys(this.state.item).length) {
      this.setState({
        ...
        isAustralianLicenceYes:
          this.state.item.is_australian_licence === 'Yes' ? true : false,
        isAustralianLicenceNo:
          this.state.item.is_australian_licence === 'No' ? true : false,
        ...
        numberOfNotAtFaultAccidents:
          this.state.item.no_of_not_at_fault_accidents,
        ...
        driverStatus: this.state.item.status,
      });
    }
  };
  ...
  callValidateDriverValidation(approve_or_reject) {
    ...
    if (this.state.bondAmount == '' || this.state.bondAmount == null) {
      ...
      this.state.bondAmountDate == '' ||
      this.state.bondAmountDate == null
    ) {
      ...
      this.state.referenceNumber == null
    ) {
  ...
  callValidateDriverApi = async approve_or_reject => {
    console.log('on submit *******', approve_or_reject);
    ...
    formData.append(
      'is_australian_licence',
      this.state.isAustralianLicenceYes ? 'Yes' : 'No',
    );
    formData.append(
      'passport_no',
      this.state.passportNo == null ? '' : this.state.passportNo,
    );
    formData.append('passport_expiry', this.state.passportExpireDate);
    formData.append(
      'utility_bill_id',
      this.state.selectedUtility == null ? '' : this.state.selectedUtility,
    );
    ...
    formData.append(
      'no_of_at_fault_accidents',
      this.state.numberOfAtFaultAccidents,
    );
    formData.append(
      'no_of_not_at_fault_accidents',
      this.state.numberOfNotAtFaultAccidents,
    );
    ...
    if (this.state.licenceImage != null && this.state.licenceImage != '')
      formData.append('licence_image', {
        uri:
          Platform.OS === 'ios'
            ? this.state.licenceImage.replace('file://', '')
            : this.state.licenceImage,
        name: 'licenceImage.jpeg',
        type: 'image/jpeg',
      });
    if (
      this.state.licenceExpiryImage != null &&
      this.state.licenceExpiryImage != ''
    )
      formData.append('licence_expiry_image', {
        uri:
          Platform.OS === 'ios'
            ? this.state.licenceExpiryImage.replace('file://', '')
            : this.state.licenceExpiryImage,
    ...
    if (this.state.passportNoImage != null && this.state.passportNoImage != '')
      formData.append('passport_no_image', {
        uri:
          Platform.OS === 'ios'
            ? this.state.passportNoImage.replace('file://', '')
            : this.state.passportNoImage,
        name: 'passportNoImage.jpeg',
        type: 'image/jpeg',
      });
    if (
      this.state.passportExpiryImage != null &&
      this.state.passportExpiryImage != ''
    )
      formData.append('passport_expiry_image', {
        uri:
          Platform.OS === 'ios'
            ? this.state.passportExpiryImage.replace('file://', '')
            : this.state.passportExpiryImage,
        name: 'passportExpiryImage.jpeg',
        type: 'image/jpeg',
      });
    if (
      this.state.utilityBillImage != null &&
      this.state.utilityBillImage != ''
    )
      formData.append('utility_bill_image', {
        uri:
          Platform.OS === 'ios'
            ? this.state.utilityBillImage.replace('file://', '')
            : this.state.utilityBillImage,
    ...
    try {
      console.log(
        'Call Validate Driver API Link ========>  ',
        Links.validateDriver,
      );
      console.log(
  ...
  editDriverApi = async () => {
    ...
    formData.append('bsb', this.state.bsb);
    formData.append('account_no', this.state.accountNumber);
    formData.append('status', this.state.driverStatus);
    formData.append(
      'no_of_at_fault_accidents',
      this.state.numberOfAtFaultAccidents,
    );
        ...
        if (responseJSON.hasOwnProperty('status') && responseJSON.status == 1) {
          if (
            ...
            for (var i = 0; i < this.state.paymentMethodList.length; i++) {
              paymentMethodListModified.push({
                value: this.state.paymentMethodList[i],
              });
            }
        ...
        <View style={styles.bottomViewContainer}>
          <View style={styles.indicatorMainContainer}>
            <View style={styles.selectedIndicatorContainer}>
              <Text numberOfLines={1} style={styles.selectedIndicatorText}>
                1
              </Text>
            </View>
          ...
          <ScrollView>
            <View>
              ...
              <Text numberOfLines={1} style={styles.headingOneTextStyle}>
                Admin Notes
              </Text>
              <View style={styles.adminNoteContainer}>
                ...
                <TextInput
                  ...
                  ref={input => {
                    this.radminNoteTextInput = input;
                  }}
                  blurOnSubmit={false}
                />
                {/* :
