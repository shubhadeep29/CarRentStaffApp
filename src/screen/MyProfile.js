    ...
    try {
      ...
      if (responseJSON) {
        ...
        if (responseJSON.hasOwnProperty('status') && responseJSON.status == 1) {
          ...
          this.setState({
            imageUrl: Links.BASEURL + responseJSON.profile_details.user_image,
          });

          await AsyncStorage.setItem(
            Constants.STORAGE_KEY_NAME,
            this.state.fullName,
          );
          await AsyncStorage.setItem(
            Constants.STORAGE_KEY_EMAIL,
            this.state.email,
          );
          await AsyncStorage.setItem(
            Constants.STORAGE_KEY_MOBILEL,
            this.state.mobile,
          );
          ...
          await AsyncStorage.setItem(
            Constants.STORAGE_KEY_GENDER,
            this.state.gender,
          );
          await AsyncStorage.setItem(
            Constants.STORAGE_KEY_ROLE,
            this.state.role,
          );
          await AsyncStorage.setItem(
            Constants.STORAGE_KEY_USER_IMAGE,
            this.state.imageUrl,
          );
        ...
        <ScrollView>
          <View style={styles.bottomViewContainer}>
            ...
            <View style={styles.column}>
              <Text numberOfLines={1} style={styles.headingBigTextStyle}>
                {this.state.fullName}
              </Text>
              <Text numberOfLines={1} style={styles.headingSmallTextStyle}>
                {this.state.mobile}
              </Text>
              <Text numberOfLines={1} style={styles.headingSmallTextStyle}>
                {this.state.email}
            ...
            <View style={styles.searchEditTextContainer}>
              <Text style={styles.infoHeadingTextStyle}>TFN</Text>
